...
export const Sidebar = ({ isExpanded, setIsExpanded }) => {
  ...
  const navItems = [
    ...
    { icon: BookOpen, label: "Guia Operacional", path: "/painel/guia", show: true },
    { icon: GraduationCap, label: "Treinamentos", path: "/painel/treinamentos", show: true },
  ...
  const isItemActive = (itemPath, isActive) => {
    if (itemPath === "/painel/casos" && location.pathname.includes("/arquivados")) {
      return false;
    }
    return isActive;
  };

  const mobileLinkClass = (itemPath) => ({ isActive }) => {
    const active = isItemActive(itemPath, isActive);
    return `flex flex-col items-center gap-1 text-xs font-medium ${
      active ? "text-primary" : "text-muted"
    }`;
  };

  const mobileIconClass = (itemPath, isActive) => {
    const active = isItemActive(itemPath, isActive);
    return `h-10 w-10 rounded-2xl flex items-center justify-center ${
  ...
  return (
    <>
              ...
              <li key={item.label} className="relative group">
                <NavLink
                  ...
                  className={({ isActive }) => {
                    ...
                    return `relative flex items-center gap-3 px-3 py-3 rounded-lg transition-colors overflow-visible ${
                      active
                        ...
                        {!isExpanded && (
                          <div 
                            className={`absolute left-0 top-0 h-full w-64 flex items-center gap-3 px-3 rounded-lg shadow-xl opacity-0 group-hover:opacity-100 pointer-events-none z-50 transition-all duration-200 border border-soft ${
                              active ? "bg-primary text-white border-primary" : "bg-surface text-main"
                            }`}
                          >
                            <div className="min-w-6 flex justify-center shrink-0">
                              <item.icon size={22} className={active ? "text-white" : "text-main"} />
                            </div>
      ...
      <nav className="lg:hidden fixed bottom-0 left-0 right-0 bg-surface/95 border-t border-soft backdrop-blur px-2 py-2 flex items-center justify-around z-40">
        {navItems.map((item) => (
          <NavLink key={item.path} to={item.path} end={item.path === "/painel"} className={mobileLinkClass(item.path)}>
            {({ isActive }) => (
              <>
                <div className={mobileIconClass(item.path, isActive)}>
                  <item.icon size={20} />
                </div>
