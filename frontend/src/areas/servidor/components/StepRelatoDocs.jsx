      ...
      <section className="form-section">
        ...
        {!configAcao?.ocultarRelato && (
          <div>
            ...
            <div className="flex justify-between mt-1 px-1">
              <span className="text-xs text-error font-medium">{formErrors.relato}</span>
            </div>
          </div>
        )}
        ...
        <div className="bg-amber-50 border border-amber-200 rounded-xl p-4 mb-4">
          <label className="flex items-center gap-3 cursor-pointer">
              ...
              onChange={async (e) => {
                ...
                if (checked) {
                  const ok = await confirm(
                    "Ao marcar esta opção, você poderá enviar o formulário sem os documentos agora. No entanto, o  atendimento SÓ SERÁ INICIADO após o envio dos documentos. Deseja continuar?",
            ...
            />
            <div>
              <span className="font-bold text-amber-900 block">Enviar documentos depois</span>
              <span className="text-xs text-amber-700">
                Entendo que meu caso só será processado após a entrega dos documentos.
              </span>
            </div>
          </label>
        </div>
